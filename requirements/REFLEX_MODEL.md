# Open-Think-Reflex 需求文档

> **版本**: v2.0  
> **状态**: 草稿  
> **定位**: v1.0 - AI输入加速器 | v2.0 - 思绪整理扩展

---

## 目录

1. [核心概念](#1-核心概念)
2. [愿景与路线](#2-愿景与路线)
3. [交互设计](#3-交互设计)
4. [反射生命周期](#4-反射生命周期)
5. [功能需求](#5-功能需求)
6. [数据结构](#6-数据结构)
7. [存储设计](#7-存储设计)
8. [匹配算法](#8-匹配算法)
9. [AI集成](#9-ai集成)
10. [导出与同步](#10-导出与同步)
11. [配置管理](#11-配置管理)
12. [错误处理](#12-错误处理)
13. [性能要求](#13-性能要求)
14. [命令行使用](#14-命令行使用)
15. [反射工作流](#15-反射工作流核心流程)
16. [Shell自动补全](#16-shell自动补全)
17. [安装与部署](#17-安装与部署)
18. [测试策略](#18-测试策略)
19. [路线规划](#19-路线规划)

---

## 1. 核心概念

### 1.1 大脑与反射的类比

```
人类神经系统              AI交互系统
─────────────────────────────────────────────
大脑（复杂思考）       AI大模型（通用智能）
  ↓                      ↓
脊髓反射（自动化）       反射（高频操作快速响应）
```

**核心观点**：
- 大脑 = 大模型（慢速思考）
- 反射 = 快捷指令（快速执行）
- 反射形成需要重复强化
- 反射衰减因为不使用

---

## 2. 愿景与路线

### 2.1 v1.0 目标：AI输入加速器

| 功能 | 优先级 | 状态 | 说明 |
|------|--------|------|------|
| 反射形成 | P0 | 🔶 | 匹配→强化→阈值→激活 |
| 思维链树 | P0 | 🔶 | 可视化展示匹配分支 |
| 快捷键触发 | P0 | 🔶 | 空格触发生成 |
| 本地存储 | P1 | 🔶 | SQLite |
| 衰减机制 | P2 | 🔶 | 长期不用自动衰减 |
| 匹配算法 | P0 | 🔶 | 语义/关键词/向量 |
| 配置管理 | P1 | 🔶 | AI provider、存储路径 |

### 2.2 v2.0 规划：思绪整理扩展

| 功能 | 说明 |
|------|------|
| 项目空间 | 不同项目/领域隔离反射集 |
| 思绪整理 | 时间线回溯、节点连接 |
| 经验导出 | 将反射导出为JSON/Skill |
| 云同步 | 多端同步反射数据 |

### 2.3 应用领域

反射系统适用于任何需要**重复思考**的领域：

| 领域 | 示例 | 说明 |
|------|------|------|
| **思考与创作** | 文章大纲、内容框架、文案模板 | 写作创作、内容结构 |
| **分析决策** | SWOT分析、根因分析、对比分析 | 商业分析、策略制定 |
| **问题解决** | 故障排查、诊断框架、复盘模板 | 问题诊断、解决方案 |
| **学习研究** | 概念解释、技术对比、学习路径 | 知识获取、技能提升 |
| **文档编写** | 项目文档、流程文档、复盘总结 | 知识沉淀、经验传承 |

### 2.4 v1.0 验收标准

```
✓ 里程碑1：核心交互
  ├── 输入关键词能触发反射匹配
  ├── 思维链树正确展示匹配分支
  └── 空格键能触发生成

✓ 里程碑2：反射形成
  ├── 重复使用能增强反射强度
  ├── 强度达到阈值自动激活
  └── 新反射能正确创建

✓ 里程碑3：持久化
  ├── 反射数据保存到SQLite
  ├── 重启后数据不丢失
  └── 能导出/导入JSON

✓ 里程碑4：衰减（可选）
  ├── 长期未用反射自动衰减
  └── 衰减参数可配置
```

---

## 3. 交互设计

### 3.1 界面布局

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                      │
│  上层：思维链树展示层（从左到右展开）                                │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐            │
│  │  用户API │ ───► │ 分页    │ ───► │ pageSize│            │
│  │  85%    │      │ 85%     │      │ pageNum │            │
│  └─────────┘      └─────────┘      └─────────┘      │
│                                                                      │
│  视觉反馈：                                                      │
│  ├── 已选分支：高亮显示（绿色）                                   │
│  ├── 未选分支：灰色/淡化                                          │
│  └── 选中态：边框闪烁                                              │
│                                                                      │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  中层：当前输出结果层                                            │
│  ┌─────────────────────────────────────────────────────────────┐     │
│  │                                                              │     │
│  │              AI输出的结果内容...                              │     │
│  │                                                              │     │
│  └─────────────────────────────────────────────────────────────┘     │
│                                                                      │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  下层：用户输入层                                                │
│  ┌─────────────────────────────────────────────────────────────┐     │
│  │ > 用户输入命令或问题...                            [Enter] │     │
│  └─────────────────────────────────────────────────────────────┘     │
│                                                                      │
│  操作提示：[↑↓]选择分支 [→/Enter]确认展开 [←/Esc]返回 [空格]触发生成 [h]帮助 [q]退出     │
│                                                                      │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 3.2 交互流程

```
用户输入（下层）
    │
    ▼
匹配反射
    │
    ├── 有匹配 ──► 展示思维链树（上层）
    └── 无匹配 ──► 提示"无匹配，按[空格]AI生成"

选择与生成
    │
    ├── 选中分支 ──► 按[空格]AI生成该分支内容
    └── 无分支可选 ──► 按[空格]AI生成新内容

确认滚动
    │
    └── 按[→/Enter]确认当前方向，展开下一级

输出执行
    │
    └── 输出到AI或执行结果
```

### 3.3 快捷键

| 按键 | 功能 | 说明 |
|------|------|------|
| ↑/↓ | 选择分支 | 上层-选择方向 |
| → / Enter | 确认展开 | 展开选中分支 |
| ← / Esc | 返回/取消 | 回退或取消 |
| Space | 触发生成 | 无匹配或选中后执行 |
| h | 帮助 | 全局 |
| q / Ctrl+C | 退出 | 全局 |

### 3.4 视觉反馈

| 状态 | 视觉表现 |
|------|----------|
| 活跃分支 | 高亮显示 |
| 选择中 | 边框闪烁 |
| 已确认 | 绿色✓ |
| 衰减中 | 黄色⚠ |
| 丧失 | 灰色✗ |

---

## 4. 反射生命周期

### 4.1 七阶段模型

```
建立印象 ──► 强化 ──► 达成阈值 ──► 初步反射 ──► 加强
                │                                    │
                ▼                                    ▼
         【反射形成区】                          深度反射（永久）
                │                                    │
                ▼                                    ▼
         不用 → 降级 → 丧失                      持续强化
```

### 4.2 各阶段特征

| 阶段 | 强度 | 感知 | 可逆性 |
|------|------|------|--------|
| 建立印象 | 10% | 强 | 易 |
| 强化 | 30% | 中 | 可逆 |
| 达成阈值 | 50% | 弱 | 尚可 |
| 初步反射 | 70% | 无 | 可逆 |
| 加强 | 85% | 无 | 难 |
| 深度反射 | 100% | 无 | 极难 |
| 衰减 | ↓ | 弱 | 可逆 |

### 4.3 衰减公式

```
Strength(t) = Strength_initial × e^(-λ × t)

其中：
- λ = 衰减常数（因反射类型而异）
- t = 距离上次强化的时间
```

### 4.4 衰减规则

| 反射类型 | 衰减常数(λ) | 默认周期 | 可调参数 |
|----------|-------------|----------|----------|
| 短期反射 | 0.1/天 | 7天衰减50% | 可调 |
| 中期反射 | 0.01/天 | 70天衰减50% | 可调 |
| 长期反射 | 0.001/天 | 700天衰减50% | 可调 |
| 永久反射 | 0 | 不衰减 | 不适用 |

### 4.5 衰减曲线

```
强度
100% │ 深度反射 ────────────────┐
     │                             │
 85% │ 加强 ────────┐              │
     │              │ 衰减曲线    │
 70% │ 初步反射 ──┤              │ 曲线
     │              │              │
 50% │ 阈值 ──────┤              │
     │              │              │
 30% │ 强化 ──────┤              │
     │              │              │
 10% │ 建立 ──────┘              │
     │                             │
  0% └──────────────────┴───────────→ 时间
         0     7天   14天   30天
```

---

## 5. 功能需求

### 5.1 核心功能（v1.0）

| 功能 | 描述 | 优先级 |
|------|------|--------|
| 三层界面显示 | 上中下三层布局 | P0 |
| 思维链树展开 | 从左到右展示反射路径 | P0 |
| 方向选择 | 上下键选择分支 | P0 |
| 确认展开 | 右键/回车确认选择方向 | P0 |
| AI触发生成 | 无匹配时按空格生成 | P0 |
| 反射匹配 | 语义/关键词匹配用户输入 | P0 |
| 反射创建 | 从对话中提取新模式 | P0 |
| 反射强化 | 多次使用增强强度 | P1 |
| 反射存储 | 本地SQLite持久化 | P1 |
| 衰减计算 | 定期计算并更新强度 | P2 |

### 5.2 扩展功能（v2.0）

| 功能 | 描述 | 说明 |
|------|------|------|
| 项目空间 | 隔离不同项目反射集 | 目录/标签隔离 |
| 时间线回溯 | 按时间查看历史反射 | 排序、筛选 |
| 节点连接 | 反射之间的关联关系 | 图结构 |
| 经验导出 | 导出为AgentSkill | AI润色 |
| 云同步 | 多设备数据同步 | Redis/PostgreSQL |
| 多人协作 | 项目交接、共享反射 | 权限控制 |

---

## 6. 数据结构

### 6.1 反射结构

```typescript
interface Pattern {
  id: string;                    // 唯一标识
  trigger: string;               // 触发词
  response: string;             // 响应内容
  strength: number;             // 强度 0-100
  threshold: number;           // 激活阈值，默认50
  decayRate: number;           // 衰减率
  decayEnabled: boolean;        // 是否启用衰减
  connections: string[];        // 关联反射ID列表
  metadata: {
    created: number;           // 创建时间戳
    updated: number;           // 更新时间戳
    reinforcementCount: number; // 强化次数
    decayCount: number;        // 衰减次数
    tags: string[];            // 标签（用于项目空间）
    project?: string;          // 项目ID（v2.0）
    userId?: string;           // 用户ID（v2.0）
  };
}
```

### 6.2 思维链树

```typescript
interface ThoughtChain {
  root: string;                 // 用户输入（根节点）
  branches: ChainBranch[];       // 分支列表
  currentDepth: number;         // 当前深度
  maxDepth: number;             // 最大深度
  createdAt: number;           // 创建时间
}

interface ChainBranch {
  id: string;
  pattern: Pattern;            // 关联反射
  next: ChainBranch[];        // 子分支
  selected: boolean;          // 是否被选择
  confirmed: boolean;         // 是否已确认展开
}
```

### 6.3 项目空间（v2.0）

```typescript
interface ProjectSpace {
  id: string;
  name: string;                // 项目名称
  description: string;         // 项目描述
  patterns: string[];          // 反射ID列表
  createdAt: number;
  updatedAt: number;
  owner: string;              // 所有者
  collaborators: string[];     // 协作者
  visibility: 'private' | 'team' | 'public';
}
```

---

## 7. 存储设计

### 7.1 SQLite存储（v1.0）

SQLite是当前推荐的存储方式，单文件、易备份。

```sql
-- 反射表
CREATE TABLE patterns (
  id TEXT PRIMARY KEY,
  trigger TEXT NOT NULL,
  response TEXT NOT NULL,
  strength REAL NOT NULL DEFAULT 0,
  threshold REAL NOT NULL DEFAULT 50,
  decay_rate REAL NOT NULL DEFAULT 0.01,
  decay_enabled INTEGER NOT NULL DEFAULT 1,
  connections TEXT,             -- JSON数组
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL,
  reinforcement_count INTEGER NOT NULL DEFAULT 0,
  decay_count INTEGER NOT NULL DEFAULT 0,
  tags TEXT,                    -- JSON数组
  project TEXT,
  user_id TEXT
);

-- 思维链表
CREATE TABLE thought_chains (
  id TEXT PRIMARY KEY,
  root TEXT NOT NULL,
  branches TEXT NOT NULL,       -- JSON
  current_depth INTEGER NOT NULL,
  max_depth INTEGER NOT NULL,
  created_at INTEGER NOT NULL
);

-- 项目空间表（v2.0）
CREATE TABLE project_spaces (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  description TEXT,
  owner TEXT NOT NULL,
  collaborators TEXT,           -- JSON数组
  visibility TEXT DEFAULT 'private',
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL
);

-- 索引
CREATE INDEX idx_patterns_trigger ON patterns(trigger);
CREATE INDEX idx_patterns_strength ON patterns(strength);
CREATE INDEX idx_patterns_project ON patterns(project);
CREATE INDEX idx_patterns_tags ON patterns(tags);
CREATE INDEX idx_chains_created ON thought_chains(created_at);
```

### 7.2 存储迁移路径

| 版本 | 存储方式 | 说明 |
|------|----------|------|
| v1.0 | SQLite | 当前推荐，单文件、易备份 |
| v1.5 | + Redis缓存 | 可选，加速热点查询 |
| v2.0 | PostgreSQL | 可选，多人协作、云同步 |

### 7.3 数据目录结构

```
~/.openclaw/reflex/
├── config.yaml              # 配置文件
├── reflex.db               # SQLite数据库
├── exports/                # 导出目录
│   ├── patterns.json
│   └── skills/
└── logs/                   # 日志目录
    └── reflex.log
```

### 7.4 备份与恢复

#### 备份命令

```bash
# 完整备份（数据库+配置）
otr backup --output reflex-backup-$(date +%Y%m%d).tar.gz

# 仅备份数据库
otr backup --db-only --output reflex-db-$(date +%Y%m%d).db

# 仅备份反射数据（JSON格式）
otr backup --json-only --output reflex-patterns-$(date +%Y%m%d).json

# 备份指定项目
otr backup --project my-project --output project-backup.tar.gz

# 自动备份（每日凌晨2点）
otr backup --schedule daily --time 02:00

# 保留最近10个备份
otr backup --retain 10
```

#### 恢复命令

```bash
# 从完整备份恢复
otr restore reflex-backup-20240220.tar.gz

# 从数据库备份恢复
otr restore --db-only reflex-db-20240220.db

# 从JSON恢复（合并到现有数据）
otr restore --merge reflex-patterns-20240220.json

# 从JSON恢复（替换现有数据）
otr restore --replace reflex-patterns-20240220.json

# 恢复前预览（不执行）
otr restore --dry-run reflex-backup-20240220.tar.gz
```

#### 备份配置

```yaml
backup:
  enabled: true
  schedule: "daily"           # daily | weekly | none
  time: "02:00"               # 备份时间（每日）
  retain: 7                   # 保留数量
  include:
    - db                     # 数据库
    - config                 # 配置
    - exports                # 导出目录
  exclude:
    - "*.log"               # 排除日志
  storage: "local"            # local | cloud
  cloud:
    provider: "s3"           # s3 | gcs | azure
    bucket: "reflex-backups"
    path: "backups/"
    credentials:
      accessKey: "${CLOUD_ACCESS_KEY}"
      secretKey: "${CLOUD_SECRET_KEY}"
```

### 7.5 版本迁移

当数据结构变更时，需要执行版本迁移。

```bash
# 查看当前版本
otr version

# 检查新版本
otr version --check

# 执行迁移
otr migrate

# 回滚到上一版本（谨慎使用）
otr migrate --rollback

# 查看迁移日志
otr migrate --log
```

#### 迁移策略

| 场景 | 处理方式 |
|------|----------|
| 数据库结构变更 | 自动创建迁移脚本、执行ALTER TABLE |
| 字段重命名 | 创建新字段、迁移数据、删除旧字段 |
| 数据格式升级 | 导出→转换→导入 |
| 跨版本跳跃 | 按顺序执行每个版本的迁移 |

#### 迁移流程

```
1. 检测当前版本
2. 下载对应迁移脚本
3. 创建数据库快照（自动）
4. 执行迁移
5. 验证数据完整性
6. 更新版本号

如果迁移失败：
1. 自动回滚到快照
2. 记录错误日志
3. 提示用户手动处理
```

#### 版本历史

| 版本 | 变更说明 |
|------|----------|
| v1.0.0 | 初始版本 |
| v1.1.0 | 新增tags字段、新增project字段 |
| v1.2.0 | 新增connections JSON字段、优化索引 |
| v2.0.0 | 新增项目空间表、新增思维链表 |

---

## 8. 匹配算法

### 8.1 匹配策略

| 策略 | 优先级 | 适用场景 | 性能 |
|------|--------|----------|------|
| exact | P0 | 精确匹配关键词 | 快 |
| keyword | P0 | 关键词提取匹配 | 快 |
| semantic | P1 | 语义相似度匹配 | 中 |
| fuzzy | P2 | 模糊匹配 | 慢 |

### 8.2 匹配流程

```
用户输入
    │
    ▼
1. exact匹配（关键词完全匹配）
    │
    ├── 命中 ──► 返回结果
    └── 未命中 ──► 下一步
        │
        ▼
2. keyword匹配（关键词提取）
    │
    ├── 命中 ──► 返回结果
    └── 未命中 ──► 下一步
        │
        ▼
3. semantic匹配（语义相似度）
    │
    ├── 相似度 ≥ 80% ──► 返回结果
    └── 相似度 < 80% ──► 无匹配
```

### 8.3 匹配配置

```yaml
matching:
  strategies:
    - name: exact
      enabled: true
      weight: 1.0
    - name: keyword
      enabled: true
      weight: 0.8
    - name: semantic
      enabled: true
      weight: 0.6
      model: "embedding-model"  # 可选
    - name: fuzzy
      enabled: false
      weight: 0.4
  threshold:
    exact: 100
    keyword: 80
    semantic: 70
    fuzzy: 60
  maxResults: 10
```

### 8.4 语义匹配（可选）

```typescript
interface SemanticMatcher {
  // 加载embedding模型
  loadModel(model: string): Promise<void>;
  
  // 生成embedding
  embed(text: string): Promise<number[]>;
  
  // 计算相似度
  cosineSimilarity(a: number[], b: number[]): number;
  
  // 批量匹配
  match(input: string, patterns: Pattern[], threshold: number): MatchResult[];
}
```

---

## 9. AI集成

### 9.1 AI提供商抽象层

不绑定特定AI提供商，支持多种AI服务。

```typescript
interface AIProvider {
  name: string;
  version: string;
  
  // 文本生成
  complete(prompt: string, options?: AIOptions): Promise<string>;
  
  // 流式生成
  stream(prompt: string, options?: AIOptions): AsyncIterator<string>;
  
  // Embedding生成
  embed(text: string): Promise<number[]>;
  
  // 健康检查
  healthCheck(): Promise<boolean>;
}

interface AIOptions {
  model?: string;
  maxTokens?: number;
  temperature?: number;
  systemPrompt?: string;
}
```

### 9.2 支持的提供商

| 提供商 | 状态 | 说明 |
|--------|------|------|
| Claude | ✅ 当前推荐 | Anthropic API |
| OpenAI | ⏳ 后续 | GPT-4 API |
| 本地模型 | ⏳ 后续 | Ollama/LM Studio |
| OpenRouter | ⏳ 后续 | 聚合多个模型 |

### 9.3 Provider配置

```yaml
providers:
  default: "claude"
  
  claude:
    apiKey: "${ANTHROPIC_API_KEY}"
    baseUrl: "https://api.anthropic.com"
    model: "claude-sonnet-4-20250514"
    maxTokens: 4096
    temperature: 0.7
  
  openai:
    apiKey: "${OPENAI_API_KEY}"
    baseUrl: "https://api.openai.com/v1"
    model: "gpt-4"
    maxTokens: 4096
    temperature: 0.7
  
  local:
    baseUrl: "http://localhost:11434/v1"
    model: "llama3"
    maxTokens: 4096
    temperature: 0.7
```

---

## 10. 导出与同步

### 10.1 导出功能

```bash
# 导出为JSON
otr export json --output patterns.json

# 导出为SQLite
otr export sqlite --output patterns.db

# 导出指定项目
otr export json --project my-project --output project-reflex.json

# 导出全部
otr export json --all --output all-reflex.json
```

### 10.2 导入功能

```bash
# 导入JSON文件
otr import json reflex-file.json

# 导入并指定到项目
otr import json reflex-file.json --project my-project

# 预览导入
otr import json reflex-file.json --preview

# 合并导入（跳过已存在）
otr import json reflex-file.json --merge

# 替换导入（覆盖已存在）
otr import json reflex-file.json --replace
```

### 10.3 JSON格式

```json
{
  "version": "1.0",
  "exportedAt": 1700000000,
  "source": "personal",
  "patterns": [
    {
      "trigger": "market-analysis",
      "response": "# Market Trend Analysis Framework\n...",
      "tags": ["analysis", "business"],
      "strength": 85,
      "createdAt": 1700000000,
      "lastUsedAt": 1700000000
    }
  ]
}
```

### 10.4 v2.0 同步规划

| 功能 | 说明 | 状态 |
|------|------|------|
| 文件导出/导入 | JSON导出导入 | ✅ v1.0 |
| 项目导出 | 项目级别导出 | ✅ v1.0 |
| 云同步 | 多设备数据同步 | ⏳ v2.0 |
| P2P同步 | 局域网同步 | ⏳ v2.0 |

### 10.5 v2.0 项目空间

```bash
# 创建项目
otr project create my-project --name "项目名"

# 列出项目
otr project list

# 切换项目
otr project use my-project

# 导出项目
otr project export my-project --output project.json

# 导入项目
otr project import project.json

## 11. 配置管理

### 11.1 配置文件结构

```yaml
# ~/.openclaw/reflex/config.yaml

# 应用配置
app:
  name: "open-think-reflex"
  version: "1.0.0"
  dataDir: "~/.openclaw/reflex"
  logLevel: "info"

# 存储配置
storage:
  type: "sqlite"              # sqlite | redis | postgresql
  sqlite:
    path: "~/.openclaw/reflex/reflex.db"
  redis:
    host: "localhost"
    port: 6379
    keyPrefix: "reflex:"
  postgresql:
    host: "localhost"
    port: 5432
    database: "reflex"
    tablePrefix: "reflex_"

# AI配置
ai:
  provider: "claude"           # 默认provider
  providers:
    claude:
      apiKey: "${ANTHROPIC_API_KEY}"
      baseUrl: "https://api.anthropic.com"
      model: "claude-sonnet-4-20250514"
      maxTokens: 4096
      temperature: 0.7

# 匹配配置
matching:
  strategies:
    exact:
      enabled: true
      weight: 1.0
    keyword:
      enabled: true
      weight: 0.8
    semantic:
      enabled: false
      weight: 0.6
      model: "embedding-model"
    fuzzy:
      enabled: false
      weight: 0.4
  threshold:
    exact: 100
    keyword: 80
    semantic: 70
    fuzzy: 60
  maxResults: 10

# 衰减配置
decay:
  enabled: true
  interval: "1h"               # 衰减计算间隔
  types:
    short:
      lambda: 0.1              # 7天衰减50%
      default: true
    medium:
      lambda: 0.01            # 70天衰减50%
    long:
      lambda: 0.001           # 700天衰减50%
    permanent:
      lambda: 0                # 不衰减

# 项目配置（v2.0）
project:
  defaultProject: null
  storageType: "directory"    # directory | database

# 导出配置（v2.0）
export:
  skill:
    outputDir: "~/reflex-skills"
    autoPublish: false
    aiPolish: true
```

### 11.2 环境变量

| 变量 | 说明 | 必填 |
|------|------|------|
| ANTHROPIC_API_KEY | Anthropic API密钥 | 当使用Claude时 |
| OPENAI_API_KEY | OpenAI API密钥 | 当使用OpenAI时 |
| REFLEX_DATA_DIR | 数据目录 | 否，默认~/.openclaw/reflex |
| REFLEX_LOG_LEVEL | 日志级别 | 否，默认info |

### 11.3 配置命令

```bash
# 查看当前配置
otr config show

# 查看特定配置项
otr config get ai.provider

# 设置配置项
otr config set matching.threshold.semantic 75

# 重置配置
otr config reset

# 导入配置
otr config import config.yaml

# 导出配置
otr config export config.yaml
```

---

## 12. 错误处理

### 12.1 错误码定义

| 错误码 | 类型 | 说明 |
|--------|------|------|
| P-001 | Pattern | 反射不存在 |
| P-002 | Pattern | 反射创建失败 |
| P-003 | Pattern | 反射更新失败 |
| P-004 | Pattern | 反射删除失败 |
| P-005 | Pattern | 反射ID重复 |
| M-001 | Matching | 匹配超时 |
| M-002 | Matching | 匹配失败 |
| S-001 | Storage | 存储空间不足 |
| S-002 | Storage | 数据库连接失败 |
| S-003 | Storage | 数据读取失败 |
| A-001 | AI | API调用失败 |
| A-002 | AI | API配额不足 |
| A-003 | AI | 模型不支持 |
| C-001 | Config | 配置文件错误 |
| C-002 | Config | 配置项不存在 |
| E-001 | Export | 导出格式不支持 |
| E-002 | Export | Skill润色失败 |

### 12.2 错误处理策略

| 场景 | 处理方式 |
|------|----------|
| 反射匹配超时 | 降级到关键词匹配 → 仍失败则提示 |
| AI API调用失败 | 重试3次 → 仍失败则使用缓存或提示 |
| 数据库连接失败 | 使用内存缓存 → 恢复后同步 |
| 配置错误 | 使用默认值 → 记录日志 |
| 存储空间不足 | 清理临时文件 → 提示用户 |
| Skill润色失败 | 使用原始内容 → 标记为未润色 |

### 12.3 错误响应示例

```typescript
interface ReflexError {
  code: string;                 // 错误码
  message: string;              // 用户友好的错误信息
  details?: Record<string, any>; // 详细信息
  suggestion?: string;          // 建议操作
  recoverable: boolean;         // 是否可恢复
  retryable: boolean;           // 是否可重试
}

// 示例
{
  "code": "A-001",
  "message": "AI服务调用失败",
  "details": {
    "provider": "claude",
    "error": "Connection timeout"
  },
  "suggestion": "请检查网络连接，或尝试切换AI提供商",
  "recoverable": true,
  "retryable": true
}
```

---

## 13. 性能要求

### 13.1 响应时间

| 操作 | 目标时间 | 最大时间 |
|------|----------|----------|
| 反射匹配（本地） | < 50ms | < 200ms |
| 反射匹配（含语义） | < 200ms | < 500ms |
| 反射创建 | < 100ms | < 300ms |
| 思维链渲染 | < 50ms | < 100ms |
| AI生成（首字） | < 1s | < 3s |
| AI生成（完成） | < 10s | < 30s |
| 数据导出 | < 1s | < 5s |

### 13.2 资源限制

| 资源 | 限制 |
|------|------|
| 反射数量 | 无限制（建议<10000） |
| 单个反射大小 | < 10KB |
| 并发请求数 | 1（CLI场景） |
| 内存占用 | < 100MB |
| 磁盘占用 | < 1GB |

### 13.3 性能监控

```bash
# 查看性能统计
otr stats

# 示例输出
反射总数: 156
今日新增: 3
今日强化: 12
匹配次数: 89
平均匹配时间: 23ms
AI调用次数: 45
失败次数: 2
```

---

## 14. 命令行使用

### 14.1 基本命令

```bash
# 启动交互模式
otr

# 直接输入问题
otr "写个用户API"

# 查看帮助
otr --help

# 查看版本
otr --version

# 导出反射
otr export --format json --output patterns.json

# 导入反射
otr import patterns.json

# 衰减配置
otr decay status
otr decay set user_api_pagination --rate 0.05

# 性能统计
otr stats

# 清除数据
otr clear --confirm
```

### 14.2 匹配相关

```bash
# 测试匹配
otr match "用户API"

# 查看所有反射
otr pattern list

# 查看单个反射详情
otr pattern show <pattern-id>

# 删除反射
otr pattern delete <pattern-id>

# 手动强化反射
otr pattern reinforce <pattern-id>
```

### 14.3 项目相关（v2.0）

```bash
# 创建项目
otr project create my-project --name "项目名" --desc "描述"

# 列出项目
otr project list

# 切换项目
otr project use my-project

# 项目成员管理
otr project add-member my-project --user alice --role editor
otr project remove-member my-project alice

# 导出项目为Skill
otr project export-skill my-project --output skills/
```

### 14.4 导出相关（v2.0）

```bash
# 导出为Skill并预览
otr export skill --source personal --preview

# 导出为Skill并保存
otr export skill --source personal --output my-skills/

# 上传到Skill市场
otr export skill --source project my-project --publish

# 列出已导出的Skill
otr export skill list --dir my-skills/
```

### 14.5 配置相关

```bash
# 查看配置
otr config show

# 查看AI提供商
otr config get ai.provider

# 设置AI提供商
otr config set ai.provider openai

# 重置配置
otr config reset
```

### 14.6 交互模式操作

```
> otr

┌─────────────────────────────────────────┐
│ 上层：思维链                              │
│ [根] ──► [分支A] ──► [分支A1] ✓          │
│ [根] ──► [分支B] ──► [分支B1] ◉          │
└─────────────────────────────────────────┤
│ 中层：输出结果                           │
│ AI: 这是根据你的选择生成的代码...        │
└─────────────────────────────────────────┤
│ 下层：输入                               │
│ > _ [按↑↓选择,→确认,空格AI生成]          │
└─────────────────────────────────────────┘
```

---

## 15. 反射工作流（核心流程）

### 15.1 反射创建流程

当用户输入无匹配的关键词时，会触发AI生成。AI生成结果后，需要决定是否将结果保存为新反射。

```
用户输入（下层）
    │
    ▼
匹配反射
    │
    ├── 有匹配 ──► 展示思维链树（上层）
    └── 无匹配 ──► 提示"无匹配，按[空格]AI生成"
                │
                ▼
         按[空格]AI生成
                │
                ▼
         生成结果
                │
                ▼
         用户选择行为
                │
                ├── 选择分支 ──► 执行该分支内容
                │           └── 自动强化该反射
                │
                └── 新内容 ──► 是否保存为新反射？
                            │
                            ├── 是 ──► 提取Pattern
                            │           └── 保存到数据库
                            └── 否 ──► 丢弃
```

#### 自动创建 vs 手动创建

| 方式 | 触发条件 | 适用场景 |
|------|----------|----------|
| **自动创建** | 用户同意保存 | 高频操作 |
| **手动创建** | 用户主动触发 `otr pattern create` | 特定模式 |

#### 反射创建命令

```bash
# 手动创建反射
otr pattern create --trigger "用户API分页" --response "SELECT * FROM users LIMIT ? OFFSET ?" --tags "api,database"

# 从对话中提取反射
otr pattern extract --from "对话内容" --auto

# 创建时指定强度
otr pattern create --trigger "xxx" --response "xxx" --strength 50

# 创建时指定衰减类型
otr pattern create --trigger "xxx" --response "xxx" --decay-type long
```

#### Pattern提取规则

```typescript
interface ExtractionRule {
  // 从AI生成结果中提取Pattern的规则
  patterns: {
    // 代码片段 → trigger为注释/函数名，response为代码
    code: {
      triggerPattern: /^\/\/ (.+)$/m,  // 以"// "开头的注释
      responsePattern: /```[\s\S]*?```/,  // 代码块
    },
    // 步骤列表 → trigger为步骤名，response为步骤内容
    steps: {
      triggerPattern: /^\d+\. (.+)$/,  // 数字开头的步骤
      responsePattern: /[\s\S]*?(?=\d+\.|$)/,
    },
    // 问答对 → trigger为问题，response为答案
    qa: {
      triggerPattern: /^[Qq](?:uestion)?:?\s*(.+)$/,
      responsePattern: /^[Aa](?:nswer)?:?\s*([\s\S]*?)(?=$|Q:)/,
    },
  },
}
```

### 15.2 反射强化流程

当用户选择并执行某个分支后，该反射的强度应增加。

```
用户选择分支
    │
    ▼
执行该分支内容
    │
    ▼
自动强化
    │
    ▼
Strength_new = Strength_old + reinforcementAmount
    │
    ▼
如果 Strength >= Threshold → 激活该反射
```

#### 强化规则

| 场景 | 强化幅度 |
|------|----------|
| 用户选择并执行分支 | +5 |
| 用户多次执行同一分支 | +10 |
| 用户修改分支内容后执行 | +8 |
| 分支达到深度反射（100%） | 不再强化 |

#### 手动强化

```bash
# 手动强化反射
otr pattern reinforce <pattern-id>

# 查看强化历史
otr pattern history <pattern-id>

# 设置强化幅度
otr config set reinforcement.positive 10
otr config set reinforcement.negative -5
```

### 15.3 衰减触发机制

衰减可以通过定时任务或访问时触发。

```
衰减触发方式：
    │
    ├── 定时触发（推荐）
    │       └── 每隔interval执行一次衰减计算
    │           └── 后台运行，不影响用户体验
    │
    └── 访问时触发
            └── 用户查询时检查并衰减
                └── 优点：实时
                └── 缺点：首次访问慢
```

#### 衰减命令

```bash
# 手动触发衰减（管理员用）
otr decay run

# 查看衰减状态
otr decay status

# 查看即将衰减的反射
otr decay preview

# 设置衰减参数
otr decay set <pattern-id> --rate 0.05

# 禁用特定反射的衰减
otr decay disable <pattern-id>

# 启用衰减
otr decay enable <pattern-id>
```

### 15.4 反射匹配与执行完整流程

```
用户输入
    │
    ▼
获取当前项目/空间的反射集
    │
    ▼
执行匹配流程（8.2）
    │
    ├── 有匹配 ──► 展示思维链树（上层）
    │           └── 按强度排序
    │
    └── 无匹配 ──► 提示"无匹配，按[空格]AI生成"
                │
                ▼
         用户按[空格]
                │
                ▼
         AI生成结果
                │
                ▼
         用户选择
                │
                ├── 选择已有分支
                │           └── 执行内容
                │               └── 自动强化该反射
                │                   └── 如果Strength >= Threshold → 标记为激活
                │
                └── 放弃选择
                            └── 提示"是否保存为新反射？"
                                │
                                ├── 是 ──► 创建新反射
                                └── 否 ──► 丢弃
```

### 15.5 项目空间与反射的关系

v1.0虽然没有项目空间概念，但所有反射默认存储在"默认空间"中。

```
v1.0 数据结构：
└── 默认空间（default）
    └── patterns/
        ├── pat-001
        ├── pat-002
        └── ...

v2.0 数据结构：
└── 项目空间
    ├── project-A/
    │   └── patterns/
    │       ├── pat-001
    │       └── ...
    │
    └── project-B/
        └── patterns/
            └── ...
```

#### v1.0 默认空间管理

```bash
# 查看默认空间中的反射
otr pattern list

# 导出默认空间所有反射
otr export --all --output all-patterns.json

# 清空默认空间
otr pattern clear --confirm

# 迁移反射到指定项目（v2.0）
otr pattern migrate <pattern-id> --to-project my-project
```

---

## 16. Shell自动补全

### 16.1 支持的Shell

| Shell | 状态 | 安装命令 |
|-------|------|----------|
| Bash | ✅ 默认 | `otr completion bash` |
| Zsh | ✅ 默认 | `otr completion zsh` |
| Fish | ⏳ 后续 | `otr completion fish` |
| PowerShell | ⏳ 后续 | `otr completion powershell` |

### 16.2 安装自动补全

```bash
# Bash
## 临时生效
source <(otr completion bash)

## 永久生效（用户级）
otr completion bash > ~/.bash_completion.d/otr

## 永久生效（系统级，需要sudo）
sudo otr completion bash > /etc/bash_completion.d/otr

# Zsh
## 临时生效
source <(otr completion zsh)

## 永久生效（~/.zshrc）
echo 'autoload -U compinit; compinit' >> ~/.zshrc
otr completion zsh > ~/.zsh/completion/_otr
```

### 16.3 自动补全功能

```
# 命令补全
otr [空格][Tab]          # 补全命令
otr exp[Tab]            # 补全为 export

# 子命令补全
otr export [Tab]        # 补全子命令
otr export --[Tab]      # 补全选项

# 参数补全
otr export --format [Tab]  # 补全参数值 json sqlite
otr project [Tab]         # 补全项目名

# 动态补全
otr pattern delete pat-[Tab]  # 补全反射ID
```

### 16.4 补全配置文件

```yaml
completion:
  shell: "bash"            # 当前Shell
  showHidden: false        # 显示隐藏文件
  fuzzy: true              # 模糊匹配
  sort: "alphabetical"     # alphabetical | none
  descriptions: true       # 显示描述
```

---

## 17. 安装与部署

### 17.1 安装方式

#### 方式一：npm全局安装（推荐）

```bash
# 安装
npm install -g @openclaw/reflex

# 验证安装
otr --version

# 升级
npm update -g @openclaw/reflex
```

#### 方式二：Docker

```bash
# 拉取镜像
docker pull openclaw/reflex:latest

# 运行（使用本地数据卷）
docker run -it --rm \
  -v ~/.openclaw/reflex:/root/.openclaw/reflex \
  -v $(pwd)/exports:/app/exports \
  openclaw/reflex

# Docker Compose
cat > docker-compose.yml << 'EOF'
version: '3.8'
services:
  reflex:
    image: openclaw/reflex:latest
    volumes:
      - ~/.openclaw/reflex:/root/.openclaw/reflex
      - ./exports:/app/exports
    environment:
      - ANTHROPIC_API_KEY=${ANTHROPIC_API_KEY}
    stdin_open: true
    tty: true
EOF

docker-compose run reflex
```

#### 方式三：二进制下载

```bash
# Linux (x86_64)
wget https://github.com/openclaw/open-think-reflex/releases/latest/download/otr-linux-x64.tar.gz
tar -xzf otr-linux-x64.tar.gz
./otr --version

# Linux (arm64)
wget https://github.com/openclaw/open-think-reflex/releases/latest/download/otr-linux-arm64.tar.gz
tar -xzf otr-linux-arm64.tar.gz
./otr --version

# macOS (Intel)
wget https://github.com/openclaw/open-think-reflex/releases/latest/download/otr-macos-x64.tar.gz
tar -xzf otr-macos-x64.tar.gz
./otr --version

# macOS (Apple Silicon)
wget https://github.com/openclaw/open-think-reflex/releases/latest/download/otr-macos-arm64.tar.gz
tar -xzf otr-macos-arm64.tar.gz
./otr --version

# Windows
# 下载 otr-windows-x64.zip，解压后运行 otr.exe
```

#### 方式四：从源码编译

```bash
# 克隆仓库
git clone https://github.com/ArmyClaw/open-think-reflex.git
cd open-think-reflex

# 安装依赖
npm install

# 构建
npm run build

# 链接到全局
npm link

# 运行
otr
```

### 17.2 环境要求

| 依赖 | 最低版本 | 推荐版本 |
|------|----------|----------|
| Node.js | v18.0.0 | v20.0.0+ |
| npm | v9.0.0 | v10.0.0+ |
| SQLite | 3.36.0 | 3.40.0+ |

### 17.3 初始化配置

```bash
# 首次运行（引导式配置）
otr init

# 交互式配置内容：
# 1. 选择AI提供商 (claude/openai/local)
# 2. 输入API Key
# 3. 选择数据目录
# 4. 配置匹配策略
# 5. 配置衰减参数

# 跳过引导，使用默认配置
otr init --skip

# 从配置文件初始化
otr init --config config.yaml
```

### 17.4 Docker部署

#### 开发环境

```bash
# 构建镜像
docker build -t reflex-dev .

# 运行开发模式（热重载）
docker run -it --rm \
  -v $(pwd):/app \
  -v ~/.openclaw/reflex:/root/.openclaw/reflex \
  -e NODE_ENV=development \
  reflex-dev \
  npm run dev
```

#### 生产环境

```bash
# 构建生产镜像
docker build -t reflex:latest .

# 运行
docker run -d \
  --name reflex \
  -v ~/.openclaw/reflex:/root/.openclaw/reflex \
  -v ./exports:/app/exports \
  -e ANTHROPIC_API_KEY=${ANTHROPIC_API_KEY} \
  -e LOG_LEVEL=info \
  reflex:latest

# 查看日志
docker logs -f reflex

# 重启
docker restart reflex

# 停止
docker stop reflex
```

#### Kubernetes部署

```yaml
# k8s-deployment.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: reflex
spec:
  replicas: 1
  selector:
    matchLabels:
      app: reflex
  template:
    metadata:
      labels:
        app: reflex
    spec:
      containers:
      - name: reflex
        image: openclaw/reflex:latest
        env:
        - name: ANTHROPIC_API_KEY
          valueFrom:
            secretKeyRef:
              name: reflex-secrets
              key: anthropic-api-key
        volumeMounts:
        - name: reflex-data
          mountPath: /root/.openclaw/reflex
        ports:
        - containerPort: 3000
      volumes:
      - name: reflex-data
        persistentVolumeClaim:
          claimName: reflex-pvc
---
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: reflex-pvc
spec:
  accessModes:
    - ReadWriteOnce
  resources:
    requests:
      storage: 1Gi
```

### 17.5 反向代理配置

#### Nginx

```nginx
server {
    listen 80;
    server_name reflex.example.com;

    location / {
        proxy_pass http://localhost:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }
}
```

### 17.6 健康检查

```bash
# CLI健康检查
otr health

# 输出示例
{
  "status": "healthy",
  "version": "1.0.0",
  "database": "connected",
  "storage": {
    "reflexCount": 156,
    "storageUsed": "12MB"
  },
  "aiProvider": {
    "name": "claude",
    "status": "connected",
    "quota": {
      "used": 45000,
      "limit": 100000,
      "percentage": 45
    }
  }
}

# Docker健康检查
docker inspect --format='{{.State.Health.Status}}' reflex
```

### 17.7 监控指标

```bash
# 查看详细统计
otr stats --verbose

# 输出示例
{
  "version": "1.0.0",
  "uptime": "2d 5h 30m",
  "patterns": {
    "total": 156,
    "active": 142,
    "decayed": 14
  },
  "usage": {
    "totalMatches": 1250,
    "todayMatches": 45,
    "averageMatchTime": "23ms"
  },
  "ai": {
    "totalCalls": 380,
    "todayCalls": 12,
    "failedCalls": 3
  },
  "storage": {
    "databaseSize": "12MB",
    "exportsSize": "8MB"
  }
}

# 导出指标（Prometheus格式）
otr stats --format prometheus
```

---

## 18. 测试策略

### 18.1 测试类型

| 类型 | 工具 | 覆盖范围 |
|------|------|----------|
| 单元测试 | Jest | 核心算法、数据结构 |
| 集成测试 | Jest + Supertest | API、数据库操作 |
| E2E测试 | Playwright | 交互流程、CLI命令 |
| 性能测试 | k6 | 匹配性能、并发能力 |

### 18.2 测试命令

```bash
# 运行所有测试
npm test

# 运行单元测试
npm run test:unit

# 运行集成测试
npm run test:integration

# 运行E2E测试
npm run test:e2e

# 运行性能测试
npm run test:perf

# 生成测试覆盖率报告
npm run test:coverage

# 监视模式（开发时）
npm run test:watch
```

### 18.3 测试配置

```yaml
# jest.config.js
module.exports = {
  testEnvironment: 'node',
  collectCoverageFrom: [
    'src/**/*.ts',
    '!src/**/*.d.ts',
  ],
  testMatch: [
    '**/*.test.ts',
  ],
  verbose: true,
  testTimeout: 10000,
};
```

### 18.4 CI/CD配置

```yaml
# .github/workflows/test.yml
name: Test

on:
  push:
    branches: [main, develop]
  pull_request:
    branches: [main]

jobs:
  test:
    runs-on: ubuntu-latest
    steps:
    - uses: actions/checkout@v4
    
    - name: Setup Node.js
      uses: actions/setup-node@v4
      with:
        node-version: '20'
        cache: 'npm'
    
    - name: Install dependencies
      run: npm ci
    
    - name: Run unit tests
      run: npm run test:unit
    
    - name: Run integration tests
      run: npm run test:integration
    
    - name: Run E2E tests
      run: npm run test:e2e
    
    - name: Upload coverage
      uses: codecov/codecov-action@v3
      with:
        files: ./coverage/lcov.info
```

---

## 19. 路线规划

### 19.1 v1.0：AI输入加速器

```
用户输入简短关键词
    │
    ▼
思维链树自动展开显示所有可能方向
    │
    ▼
用户选择方向，快速滚动迭代
    │
    ▼
AI生成高质量结果
```

**v1.0 目标**：
1. **反射形成** - 从高频操作中学习
2. **快速触发** - 空格键触发生成
3. **可视化思维** - 展示AI推理路径
4. **个性化** - 每个用户发展独特的反射集

### 19.2 v2.0：思绪整理扩展

| 功能 | 优先级 | 说明 |
|------|--------|------|
| 项目空间 | P0 | 隔离不同项目的反射集 |
| 时间线回溯 | P1 | 按时间查看历史反射 |
| 经验导出 | P1 | 将反射导出为AgentSkill |
| 云同步 | P2 | 多设备数据同步 |
| 多人协作 | P2 | 项目交接、共享反射 |
| Skill市场 | P2 | 导出、上架、版本管理 |

### 19.3 版本发布计划

| 版本 | 目标 | 时间 |
|------|------|------|
| v1.0.0 | 核心交互、反射形成、SQLite存储 | 3个月 |
| v1.1.0 | 语义匹配、配置管理 | 1个月 |
| v1.2.0 | 衰减机制、性能优化 | 1个月 |
| v2.0.0 | 项目空间、经验导出 | 3个月 |

---

**文档版本**: v2.1  
**更新日期**: 2026-02-20  
**项目**: open-think-reflex
